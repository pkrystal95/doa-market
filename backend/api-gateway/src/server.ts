...
import { generalLimiter, authLimiter, writeLimiter } from './middleware/rateLimiter.middleware';
import {
  requestIdMiddleware,
...
const services: ServiceConfig[] = [
  ...
  { path: '/api/v1/profiles', target: 'http://localhost:3002', auth: 'required' },

  // Product service - optional auth (public browsing, auth for modifications) + caching
  { path: '/api/v1/products', target: 'http://localhost:3003', auth: 'optional' },
  { path: '/api/v1/categories', target: 'http://localhost:3003', auth: 'optional' },

  // Order service - auth required
  { path: '/api/v1/orders', target: 'http://localhost:3004', auth: 'required' },

  // Payment service - auth required
  { path: '/api/v1/payments', target: 'http://localhost:3005', auth: 'required' },

  // Shipping service - auth required
  { path: '/api/v1/shipping', target: 'http://localhost:3006', auth: 'required' },

  // Seller service - auth required, seller or admin role
  { path: '/api/v1/sellers', target: 'http://localhost:3007', auth: 'required', roles: ['seller', 'admin'] },

  // Settlement service - auth required, seller or admin role
  { path: '/api/v1/settlements', target: 'http://localhost:3008', auth: 'required', roles: ['seller', 'admin'] },

  // Coupon service - optional auth
  { path: '/api/v1/coupons', target: 'http://localhost:3009', auth: 'optional' },

  // Inventory service - auth required
  { path: '/api/v1/inventory', target: 'http://localhost:3010', auth: 'required' },

  // Notification service - auth required
  { path: '/api/v1/notifications', target: 'http://localhost:3011', auth: 'required' },

  // Review service - optional auth
  { path: '/api/v1/reviews', target: 'http://localhost:3012', auth: 'optional' },
  ...
  { path: '/api/v1/admin', target: 'http://localhost:3014', auth: 'required', roles: ['admin'] },
...
services.forEach(({ path, target, auth, roles }) => {
  ...
  if (auth === 'required') {
    if (roles && roles.length > 0) {
      app.use(path, authMiddleware, requireRole(...roles), createProxyMiddleware(proxyOptions));
    } else {
