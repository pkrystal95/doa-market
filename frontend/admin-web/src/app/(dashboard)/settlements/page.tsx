...
import { Check, DollarSign, Eye } from 'lucide-react';
...
export default function SettlementsPage() {
  ...
  const { data: settlements, isLoading } = useQuery({
    queryKey: ['settlements', page, filter],
    queryFn: async () => {
  ...
  const confirmMutation = useMutation({
    mutationFn: (settlementId: string) => settlementsApi.confirmSettlement(settlementId),
    onSuccess: () => {
  ...
  const markAsPaidMutation = useMutation({
    mutationFn: (settlementId: string) => settlementsApi.markSettlementAsPaid(settlementId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['settlements'] });
    },
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-2">판매자 정산을 관리합니다</p>
        </div>
      </div>
              ...
              {settlements?.data?.map((settlement: Settlement) => (
                <TableRow key={settlement.id}>
                  <TableCell className="font-medium">
                    {settlement.seller?.businessName || settlement.sellerId}
                  </TableCell>
                  <TableCell>
